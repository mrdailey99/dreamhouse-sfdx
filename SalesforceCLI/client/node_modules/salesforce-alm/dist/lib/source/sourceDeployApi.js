...
class SourceDeployApi extends sourceDeployApiBase_1.SourceDeployApiBase {
    ...
    async doDeploy(options) {
        ...
        try {
            if (options.sourcepath) {
                ...
                aggregateSourceElements = await this._getSourceElementsFromManifest(options.manifest);
            }
            else if (options.metadata) {
                aggregateSourceElements = await this._getSourceElementsFromMetadata(options, tmpOutputDir, aggregateSourceElements);
            }
    ...
    async _getSourceElementsFromMetadata(options, tmpOutputDir, aggregateSourceElements) {
        const manifestPath = await SourceUtil.toManifest(this.orgApi, options, tmpOutputDir);
        this.logger.info(`Deploying metadata in metadata '${options.metadata}' from org '${this.orgApi.name}'`);
        if (manifestPath) {
            aggregateSourceElements = await this._getSourceElementsFromManifest(manifestPath);
        }
        else {
    ...
    async _getSourceElementsFromManifest(optionsManifest) {
        const typeNamePairs = await SourceUtil.parseToManifestEntriesArray(optionsManifest);
        return this._parseTypeNamePairs(typeNamePairs, this.swa.getAggregateSourceElements(false));
    }
    _parseTypeNamePairs(typeNamePairs, sourceElements) {
        let aggregateSourceElements = new Map();
        typeNamePairs.forEach((entry) => {
            if (entry.name.includes('*')) {
                let keyMetadataType = entry.type;
                const metadataType = metadataTypeFactory_1.MetadataTypeFactory.getMetadataTypeFromMetadataName(keyMetadataType, this.swa.metadataRegistry);
                if (!metadataType) {
                ...
                aggregateSourceElements = new Map([
                    ...
                    ...this._parseTypeNamePairs(wildcards, sourceElements)
                ]);
            }
            else {
                const ase = SourceUtil.loadSourceElement(sourceElements, `${entry.type}__${entry.name}`, this.swa.metadataRegistry);

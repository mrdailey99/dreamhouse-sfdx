...
exports.SALESFORCE_CERT_FINGERPRINT = process.env.SFDX_DEVELOPER_TRUSTED_FINGERPRINT ||
    '37:03:14:9C:96:FE:16:9C:96:C7:8F:DB:84:A4:E8:D5:5A:14:74:9A';
        ...
        request.on('socket', (socket) => {
            socket.on('secureConnect', () => {
                ...
                if (!_.includes(exports.SALESFORCE_CERT_FINGERPRINT, fingerprint)) {
                    throw new kit_1.NamedError('CertificateFingerprintNotMatch', `The expected fingerprint and the fingerprint [${fingerprint}] from the certificate found at https://developer.salesforce.com do not match.`);
                }
            });
        });
    }
...
function retrieveKey(stream) {
    return new Promise((resolve, reject) => {
        ...
        if (stream) {
            ...
            stream.on('end', () => {
                if (!_.includes(key, '-----BEGIN')) {
                    return reject(new kit_1.NamedError('InvalidKeyFormat', 'The specified key format is invalid.'));
                }
                return resolve(key);
            });
            stream.on('error', err => {
        ...
        codeVerifierInfo.dataToVerify.on('end', () => {
            ...
            codeVerifierInfo.signatureStream.on('end', () => {
                if (signature.byteLength === 0) {
                    return reject(new kit_1.NamedError('InvalidSignature', 'The provided signature is invalid or missing.'));
                }
                else {

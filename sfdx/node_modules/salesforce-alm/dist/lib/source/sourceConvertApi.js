...
class SourceConvertApi {
    ...
    doConvert(context) {
        ...
        return this.initWorkspaceAdapter()
            .then(() => {
            return core_1.fs.access(context.rootDir, core_1.fs.constants.R_OK);
        })
            .catch((err) => {
            if (err.code === 'ENOENT') {
                throw new core_2.SfdxError(messages.getMessage('invalidRootDirectory', [context.rootDir], 'sourceConvertCommand'));
            }
        })
            .then(() => {
            ...
            if (context.manifest) {
                ...
                return sourceElementsResolver.getSourceElementsFromMetadata(context, aggregateSourceElements);
            }
            else {

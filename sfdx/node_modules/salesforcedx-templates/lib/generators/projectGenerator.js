...
class ProjectGenerator extends generator {
    ...
    writing() {
        const { outputdir, projectname, template, defaultpackagedir, manifest, ns, sourceApiVersion, loginURL } = this.options;
        const folderlayout = [
            outputdir,
            projectname,
            defaultpackagedir,
        ...
        if (manifest === true) {
            this.fs.copyTpl(this.templatePath(manifestFile), this.destinationPath(path.join(outputdir, projectname, 'manifest', 'package.xml')), { sourceApiVersion });
        }
        // tslint:disable-next-line:no-unused-expression
        if (template === 'standard') {
            ...
            this.fs.copyTpl(this.templatePath('.eslintrc.json'), this.destinationPath(path.join(outputdir, projectname, defaultpackagedir, 'main', 'default', 'lwc', '.eslintrc.json')));
        ...
        if (template === 'analytics') {
            ...
            for (const file of filestocopy) {
                this.fs.copyTpl(this.templatePath(file), this.destinationPath(path.join(outputdir, projectname, file)));
            }
        }
    }
}
